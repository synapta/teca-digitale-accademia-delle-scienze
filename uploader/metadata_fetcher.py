...
if os.path.exists('./book.csv'):
    doneBooks = pd.read_csv('book.csv', dtype=str, encoding='utf-8')
    done = doneBooks['identifier'].tolist()
else:
    done = []
    ...
    for element in os.listdir(basedir + '/' + directory):
        if ".xml" == element[-4:] and element.replace(".xml", "") not in done:
            ...
            for child in root.find(namespace+'bib'):
                ...
                #files = glob.glob(filename.replace(".xml","") + '/*.zip')
            convert_code = os.system('convert "' + basedir + directory +  "/" + element.replace(".xml", "") + '/Archivio/*.tif" -quality 100 ' + identifier + '.pdf')
            ...
            if convert_code != 0:
                ...
                system.exit(convert_code)

            print "Uploading"
            #print bookObj
            r = upload(bookObj['identifier'],files=identifier + '.pdf', metadata=bookObj)
